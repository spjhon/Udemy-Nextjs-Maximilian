...
type TrainingsCount = number;

const hasTrainings = db.prepare('SELECT COUNT(*) as count FROM trainings').get() as TrainingsCount;

const hasTrainingsBoolean = hasTrainings > 0;


if (!hasTrainingsBoolean) {
  db.exec(`
